...
def quiz_list(request: HttpRequest):
    ...
    if request.headers.get("HX-Request"):  # Check if request is from HTMX
        return render(request, "quiz_list_partial.html", {"quizzes": quizzes})
...
def update_question(request, question_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return render(request, 'question_detail.html', {'question': question})  # Return updated question
    else:
...
def delete_question(request, pk):
    ...
    # Return an empty response to let HTMX remove the deleted element
    return HttpResponse('', headers={'HX-Trigger': 'deleteQuestion'})  